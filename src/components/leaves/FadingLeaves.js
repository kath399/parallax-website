    ...
    useEffect(() => {
        ...
        texturesPromise.then((resolvedTextures) => {
            ...
            leaves.forEach((leaf, i) => {
                ...
                leaf.spinSpeed =
                    ((randRange(0, 1) - 0.5) * blowerIntensity) / 400;
            ...
            const onContainerOver = (e) => {
                ...
                leaves.forEach((leaf, i) => {
                    if (!leaf.isBlowingAway && !leaf.isDead) {
                        if (
                            Math.abs(mousePos.x - leaf.x) < blowerRange &&
                            Math.abs(mousePos.y - leaf.y) < blowerRange
                        ) {
                            leaf.isBlowingAway = true;
                        }
                    }
