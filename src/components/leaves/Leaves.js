...
/*
  const leafSetting = {
    initialLeaves : 30,
    maxLeaves : 100,
    leafRespawnInterval : 5, //0 = no interval      
    blowerIntensity: 5,
    blopwerRange: 5,
  };
...
////
    useEffect(() => {
        ...
        console.log('====================');
        console.log('CANVAS LAUNCHED : ', canvas.width, canvas.height, window.innerWidth, window.innerHeight);
...
const randRange = (min, max) => {
        ...
        leaves = [...Array(maxLeaves).keys()].map(
            (x) => new PIXI.Sprite(resolvedTextures.leaves[leafAssetAliasList[(x%leafAssetAliasList.length)]])
        );
        shadows = [...Array(maxLeaves).keys()].map(
            (x) => new PIXI.Sprite(resolvedTextures.shadows[shadowAssetAliasList[(x%shadowAssetAliasList.length)]])
        );
        ...
        leaves.forEach((leaf, i) => {
            ...
            let leafInitPos = { x: randRange(0, canvas.width), y: randRange(0, canvas.height) };
            ...
            leaf.intensity = randRange(blowerIntensity *0.7,blowerIntensity *1.3);
            leaf.wind = {
    ...
    /*
        ...
        app.stage.addEventListener('pointermove', (e) => {
            // _container.children[0].position.copyFrom(e.global);
            // console.log(_container.children);
        });

        // for (let i = maxLeaves - 1; i >= 0; i--) {
        //     _container.addChild(this.leaves[i]);
        // }
        ...
        leaves.forEach((leaf, i) => {
            if(!leaf.isDead) {
                if(leaf.isBlowingAway) {
                    ...
                    if(leaf.x < -2000 || leaf.x > canvas.width+2000 || 
                        leaf.y < -2000 || leaf.y > canvas.height+2000
                    ) {
                        console.log("removing leaf no."+leaf.i);
                        leaf.isDead = true;
                        deadLeaves+=1;
                        increment();
                        // console.log(deadLeaves/maxLeaves, state.count);

                        if(deadLeaves/maxLeaves === 1) {
